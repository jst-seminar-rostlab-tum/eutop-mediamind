...
def schedule_jobs(service: SchedulerService, cfg: Config) -> None:
    ...
    service.schedule_periodic(
        ...
        func = job_request,
        args =[f"{cfg.API_BASE_URL}/jobs/email"],
    )
    ...
    service.schedule_periodic(
        ...
        func = job_request,
        args =[f"{cfg.API_BASE_URL}/jobs/pipeline"],
    )

def job_request(url: str):
    req = requests.get(url)
    if req.status_code == 200:
        print(f"Job triggered successfully for: {url}")
    else:
        print(f"Job failed for: {url} with status code {req.status_code}")
