...
from app.schemas.articles_schemas import MatchArticleOverviewContent, MatchDetailResponse, MatchTopicItem
...
@patch("app.services.search_profiles_service.MatchRepository.get_matches_by_search_profile", new_callable=AsyncMock)
def test_get_article_overview(
    ...
    result = asyncio.run(SearchProfileService.get_article_matches(
...
def test_get_match_detail_success():
    ...
    with patch("app.repositories.match_repository.MatchRepository.get_match_by_id", new_callable=AsyncMock) as mock_get_match, \
         patch("app.repositories.match_repository.MatchRepository.get_matches_by_profile_and_article", new_callable=AsyncMock) as mock_get_matches, \
         patch("app.repositories.search_profile_repository.SearchProfileRepository.get_search_profile_by_id", new_callable=AsyncMock) as mock_get_profile, \
         patch("app.repositories.topics_repository.TopicsRepository.get_topic_names_by_ids", new_callable=AsyncMock) as mock_get_topic_names, \
         patch("app.repositories.keyword_repository.KeywordRepository.get_keywords_by_topic_ids", new_callable=AsyncMock) as mock_get_keywords:
