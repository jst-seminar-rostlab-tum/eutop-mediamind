...
def test_get_available_search_profiles():
    with patch(
        "app.services.search_profiles_service"
        ...
        result = asyncio.run(
            SearchProfiles.get_available_search_profiles(current_user)
        )
...
def test_get_search_profile_with_access():
    with patch(
        "app.services.search_profiles_service.SearchProfileRepository."
        "get_by_id",
        new_callable=AsyncMock,
    ) as mock_get:
        ...
        result = asyncio.run(
            SearchProfiles.get_search_profile(search_profile_id, current_user)
        )
...
def test_get_article_overview():
    with patch(
        "app.services.search_profiles_service.MatchRepository"
        ".get_articles_by_profile",
        new_callable=AsyncMock,
    ) as mock_get:
        profile_id = uuid4()
...
def test_update_search_profile_success():
    with (
        ...
        result = asyncio.run(
            SearchProfiles.update_search_profile(
                profile_id, data, current_user
            )
        )
...
def test_get_match_detail_success():
    with patch(
        ...
        result = asyncio.run(
            SearchProfiles.get_match_detail(profile_id, match_id)
        )
