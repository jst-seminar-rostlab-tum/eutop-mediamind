# tests/unit/test_article_vector_service_unit.py
import uuid
import pytest
import asyncio
...
def make_article(id=None, summary="Test summary", title="Test Title", subscription_id=None):
    return Article(
        id=id or uuid.uuid4(),
        summary=summary,
        title=title,
...
def test_add_articles_calls_vector_store(patch_qdrant_and_store):
    ...
    articles = [make_article(id=uuid.UUID(int=i), subscription_id=uuid.UUID(int=i+10)) for i in range(1, 4)]
    service = ArticleVectorService()
...
@pytest.mark.asyncio
async def test_retrieve_by_similarity_delegates_to_store(patch_qdrant_and_store):
    _, store = patch_qdrant_and_store
    store.similarity_search_with_score.return_value = [(Document(page_content='x', metadata={}), 0.9)]
    service = ArticleVectorService()
    results = await service.retrieve_by_similarity('query', score_threshold=0.5)
    assert results == [(Document(page_content='x', metadata={}), 0.9)]


@pytest.mark.asyncio
async def test_add_article_with_valid_summary(monkeypatch, patch_qdrant_and_store):
    _, store = patch_qdrant_and_store
    art = make_article()
    async def fake_get(aid):
        return art
    monkeypatch.setattr(
        'app.repositories.article_repository.ArticleRepository.get_article_by_id',
        fake_get
    )
    service = ArticleVectorService()
...
@pytest.mark.asyncio
async def test_add_article_missing_or_empty_summary_logs_error(monkeypatch, caplog, patch_qdrant_and_store):
    _, store = patch_qdrant_and_store
