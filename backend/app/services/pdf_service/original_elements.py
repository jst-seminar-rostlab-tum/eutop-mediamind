...
def create_original_articles_elements(news_items, styles, translator):
    """
    Returns a list of Flowables for the appendix
    with all original-language articles.
    Each article gets an anchor for linking from
    the full article, and a backlink to the full article.
    """
    elements = []
    ...
    elements.append(
        Paragraph(f"<b>{appendix_title}</b>", styles["title_style"])
    )
    ...
    for i, news in enumerate(news_items):
        ...
        elements.append(
            Paragraph(f"<b>{news.title_original}</b>", styles["title_style"])
        )
        # Link back to full article
        elements.append(
            Paragraph(
                f"""<a href="#toc_article_{i}">
                {translator("Back to translated article")}
                </a>
                """,
                styles["button_style"],
            )
        ...
        elements.append(
            Paragraph(
                f"""<font size="10">{news.newspaper.name} |
                  {news.published_at}</font>
                  """,
        ...
        elements.append(
            Paragraph(
                f"""<a href="#toc_article_{i}">{
                    translator("Back to translated article")}</a>
                """,
                styles["button_style"],
            )
        )
