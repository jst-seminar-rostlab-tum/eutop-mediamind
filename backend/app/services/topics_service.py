...
class TopicsService:
    ...
    @staticmethod
    async def add_topic(profile_id: UUID, request: TopicCreateRequest, user: User):
    ...
    async def delete_topic(profile_id: UUID, topic_id: UUID, user: User):
        success = await TopicsRepository.delete_topic(profile_id, topic_id, user)
        if not success:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Topic not found or unauthorized.")
