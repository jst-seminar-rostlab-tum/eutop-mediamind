...
from app.services.web_harvester.login import hardcoded_login, hardcoded_logout
...
class WebHarvester:
    ...
    async def save_article(self, subscription, article):
        """Save an article using a new session to avoid greenlet conflicts"""
        ...
        engine = create_async_engine(
            "postgresql+asyncpg://postgres:postgres@localhost:5432/mediamind")
        ...
        async with async_session_maker() as new_session:
            try:
                ...
                await SubscriptionRepository.save_article(new_session, subscription, article)
                # logger.info(
    ...
    async def fetch_articles(self, uuid=None):
        subscriptions = await SubscriptionRepository.get_all_no_paywall_with_newsapi_id(self.session, uuid)
        for subscription in subscriptions:
            ...
            if articles:
                ...
                # Batch save all articles for this subscription
    ...
    async def _start_scraping(self, articles_grouped_by_subscription):
        ...
        for subscription_id, subscription_name, subscription_domain, paywall, config, articles in (
            (d['subscription_id'], d['subscription_name'],
             d['domain'], d['paywall'], d['config'], d['content'])
            for d in articles_grouped_by_subscription
        ):
            if paywall:
                task = asyncio.create_task(
                    self._scrape_paywall_articles(
                        subscription_id, subscription_name, subscription_domain, articles, config)
                )
                tasks.append(task)
            else:
                # Handle non-paywalled articles in parallel
                task = asyncio.create_task(
                    self._scrape_regular_articles(
                        subscription_id, subscription_name, articles)
                )
                tasks.append(task)
    ...
    async def _scrape_regular_articles(self, subscription_id: int, subscription_name: str, articles: List[Any]) -> dict:
        ...
        """
        logger.info(
            f"Scraping {len(articles)} regular articles for {subscription_name}")
        ...
        for article_data in articles:
            try:
                ...
                failures += 1
                logger.error(
                    f"Failed to scrape article {article_data['url']}: {e}")

        logger.info(
            f"Completed scraping for {subscription_name}: {successes} successes, {failures} failures")
    ...
    async def _scrape_single_article(self, subscription_id: int, article_data: dict, html) -> bool:
        ...
        try:
            logger.info(
                f"Scraping article with empty content: {article_data['url']}")
            ...
            article = Article(
                ...
                author=", ".join(
                    raw_article.authors) if raw_article.authors else None,
            ...
            logger.info(
                f"Scraped article {article_data['url']}: {article_text[:100]}...")
    ...
    async def _scrape_paywall_articles(self, subscription_id, subscription_name, subscription_domain, articles, config):
        ...
        """
        logger.info(
            f"Scraping {len(articles)} paywalled articles for {subscription_name}")
        ...
        login_success = hardcoded_login(
            driver, wait, subscription_name, config, subscription_domain)
        ...
        if login_success:
            ...
            for article_data in articles:
                try:
                    ...
                    failures += 1
                    logger.error(
                        f"Failed to scrape paywalled article {article_data['url']}: {e}")
            hardcoded_logout(driver, wait, config)
            driver.quit()
        else:
...
async def main():
    ...
    async with async_session() as session:
        ...
        # articles = await SubscriptionRepository.get_articles_with_empty_content_for_subscription(session, "129765e8-9aa7-4c86-8455-f1e4bfee79b5")
        # await harvester._start_scraping(articles)
        await harvester.fetch_articles("60e6484f-449f-4d68-a110-ec2dbfd138a8")
