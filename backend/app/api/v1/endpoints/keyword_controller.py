...
async def get_keywords_by_topic_id(
    ...
    """
    keywords: List[Keyword] = await KeywordRepository.get_keywords_by_topic(
...
async def create_keyword_by_topic_id(
    ...
    """
    keyword: Keyword = await KeywordRepository.create_keyword_by_topic(
        topic_id, keyword_name, current_user
    )
    return keyword
...
async def assign_keywords_to_articles(
    ...
    background_tasks.add_task(
        KeywordRepository.assign_articles_to_keywords,
        page_size,
        score_threshold,
    )
    return {
        "message": "Keywords are being assigned to articles in the background."
    }
