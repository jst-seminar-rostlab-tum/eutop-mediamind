...
@router.get("/list")
async def get_keywords_list(limit: int = 100, offset: int = 0) -> Sequence[Keyword]:

    """
    Retrieve a list of keywords with pagination.
    """

    keywords = await KeywordRepository.list_keywords(limit=limit, offset=offset)
    return keywords
...
async def get_most_similar_articles(keyword_id: UUID, score_threshold: float = 0.3) -> Sequence[Article]:
    """
