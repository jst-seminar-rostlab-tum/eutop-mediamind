...
async def add_article_to_vector_store(
    ...
    return {
        "message": f"Article {article_id} is being added to the vector store."
    }


@router.post("/add-articles")
async def add_articles_to_vector_store(background_tasks: BackgroundTasks):
    """
    ...
    return {
        "message": "All articles without summaries are being added to the vector store."
    }
