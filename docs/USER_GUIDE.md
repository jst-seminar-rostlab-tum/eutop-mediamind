...
> Note: Subscriptions are chosen on two different layers: The organization layer and the search profile layer. Organization layer subscriptions are used for sending full text articles to internal emails (specified in search profile) & display them in the frontend. Search profile layer subscriptions are used for sending full text articles to external emails (specified in search profile).
...
Within the edit/create dialog, users can configure fundamental profile settings, including the profile name and user access permissions. This is also where users define the profile's [topics and keywords](#topics--keywords).

Users can add organizational members which will receive the daily news reports generated by the search profile. Additionally, external email addresses (from outside the organization) can be added to receive daily reports.

<img src="assets/mail-settings.png" alt="Email Settings" width="500">

Users can specify which news sources should be included in reports sent to external recipients. Source filtering applies exclusively to external email recipients, internal recipients automatically receive reports containing all sources available within their organization.

<img src="assets/sub-settings.png" alt="Subscription Settings" width="500">
