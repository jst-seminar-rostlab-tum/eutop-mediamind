...
async def test_get_keyword_by_id(monkeypatch):
    monkeypatch.setattr(
        "app.repositories.keyword_repository.async_session", MagicMock()
    )
    monkeypatch.setattr(
        "app.repositories.keyword_repository.select", MagicMock()
    )
    monkeypatch.setattr(
        "app.repositories.keyword_repository.Keyword", MagicMock()
    )
...
async def test_add_keyword(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.repositories.keyword_repository.async_session",
        lambda: DummySession(),
    )
