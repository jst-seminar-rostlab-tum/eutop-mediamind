...
def test_handle_login_if_needed_paywall_success():
    ...
    wait = MagicMock()
    with patch(
        "app.services.web_harvester.utils.web_utils.hardcoded_login",
        return_value=True,
    ):
        assert (
            _handle_login_if_needed(subscription, scraper, driver, wait)
            is True
        )
...
def test_handle_login_if_needed_paywall_fail():
    ...
    with patch(
        "app.services.web_harvester.web_harvester_orchestrator.hardcoded_login",
        return_value=False,
    ):
        assert (
            _handle_login_if_needed(subscription, scraper, driver, wait)
...
def test_scrape_articles_success():
    ...
    driver = MagicMock()
    articles = [
        Article(id=1, url="http://a.com"),
        Article(id=2, url="http://b.com"),
    ]
    with (
        patch(
            "app.services.web_harvester.web_harvester_orchestrator.safe_page_load",
            return_value=True,
        ),
        patch(
            "app.services.web_harvester.web_harvester_orchestrator.get_response_code",
            return_value=200,
        ),
        patch(
            "app.services.web_harvester.web_harvester_orchestrator.safe_execute_script",
...
_handle_logout_and_cleanup = __import__(
    "app.services.web_harvester.web_harvester_orchestrator",
    fromlist=["_cleanup_driver_safe"],
...
def test_run_selenium_code_login_fail_llm_update_fail(monkeypatch):
    ...
    subscription = DummySubscription(
        paywall=True, llm_login_attempt=now - timedelta(days=8)
    )
    articles = [Article(id=1, url="http://a.com")]
    monkeypatch.setattr(
        "app.services.web_harvester.web_harvester_orchestrator._handle_login_if_needed",
        lambda *a, **k: False,
    )
    ...
    monkeypatch.setattr(
        "asyncio.run_coroutine_threadsafe", lambda *a, **k: DummyFuture()
    )
    from unittest.mock import MagicMock

    result = run_selenium_code(
        articles, subscription, subscription.scrapers, MagicMock()
    )
    assert result[0].status == ArticleStatus.ERROR
    assert "Login failed" in result[0].note


def test_run_selenium_code_login_fail_llm_update_success_but_still_fail(
    monkeypatch,
):
    from datetime import datetime, timedelta, timezone
...
def test_run_selenium_code_scraper_raises(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.services.web_harvester.web_harvester_orchestrator.create_driver",
        fake_create_driver,
    )
    monkeypatch.setattr(
        "app.services.web_harvester.web_harvester_orchestrator.safe_page_load",
...
def test_run_selenium_code_finally(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.services.web_harvester.web_harvester_orchestrator.hardcoded_logout",
        lambda *a, **k: None,
    )
    from unittest.mock import MagicMock
...
def test_handle_logout_and_cleanup_logout_raises(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.services.web_harvester.web_harvester_orchestrator.hardcoded_logout",
        lambda **kwargs: (_ for _ in ()).throw(Exception("logout error")),
    )
    driver.quit = MagicMock()
    _handle_logout_and_cleanup(driver, wait, subscription, scraper, True)
    driver.quit.assert_called_once()
