...
def test_newsapi_crawler_parse_language():
    ...
    with patch.object(
        NewsAPICrawler,
        "__init__",
        lambda self, subscription, sourceUri, include_blogs, filter_category: None,
    ):
...
def test_newsapi_get_best_matching_source_success(monkeypatch):
    ...
    with patch.object(
        ...
        monkeypatch.setattr(
            "requests.get",
            lambda url, params: MagicMock(
                json=lambda: [{"score": 60000}], raise_for_status=lambda: None
            ),
        )
...
def test_newsapi_get_best_matching_source_low_score(monkeypatch):
    ...
    with patch.object(
        ...
        monkeypatch.setattr(
            "requests.get",
            lambda url, params: MagicMock(
                json=lambda: [{"score": 100}], raise_for_status=lambda: None
            ),
        )
        result = crawler.get_best_matching_source("https://abc.com")
...
def test_newsapi_get_best_matching_source_request_exception(monkeypatch):
    ...
    with patch.object(
        ...
        monkeypatch.setattr(
            "requests.get",
            lambda url, params: MagicMock(raise_for_status=raise_exc),
        )
