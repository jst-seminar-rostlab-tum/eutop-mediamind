...
async def test_cleanup_articles_older_than_days_normal(monkeypatch):
    ...
    monkeypatch.setattr(service, "_process_article_batch", AsyncMock(return_value={"articles_deleted": 1, "entities_deleted": 1, "keyword_links_deleted": 1, "matches_deleted": 1, "vector_store_deletions": 1, "errors": 0, "batch_size": 1}))
    monkeypatch.setattr("app.services.article_cleanup_service.async_session", lambda: DummySession())
    monkeypatch.setattr("app.services.article_cleanup_service.logger", MagicMock())
    stats = await service.cleanup_articles_older_than_days(days=1, batch_size=1)
